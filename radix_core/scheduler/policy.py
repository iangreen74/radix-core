...
def score_jobs(
    queue_df: pd.DataFrame, preds: pd.DataFrame, now_s: float, cfg: PolicyConfig
) -> pd.Series:
    """
...
class PreemptionGate:
    ...
    def should_preempt(
        ...
        should_preempt = evaluate_preemption_gain(
            current_job, candidate_job, current_preds, candidate_preds, self.gain_threshold
        )
